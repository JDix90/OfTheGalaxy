...
class PlanetWorld {
  ...
  addPlayer({ id, character, ws, combatant, abilities }) {
    ...
    const player = {
      ...
      color,
      x: spawn.x, z: spawn.z, facing: spawn.facing, level: 0,
  ...
  step(dt, now) {
    ...
    for (const p of this.players.values()) {
      ...
      if (!p.dead) {
        const next = this.sim.integrate({ x: p.x, z: p.z, facing: p.facing, level: p.level }, p.input, dt);
        p.x = next.x; p.z = next.z; p.facing = next.facing;
        p.moving = next.moving; p.speed = next.speed; p.level = next.level || 0;
  ...
  stepEnemies(dt, now) {
    ...
    for (const e of this.enemies.values()) {
      ...
      for (const p of this.players.values()) {
        if (p.dead) continue;
        if (p.level) continue; // up on the rooftops → out of reach of ground enemies (a refuge)
  ...
  playersWire() {
    ...
    for (const p of this.players.values()) {
      out.push({
        ...
        ...(p.level ? { l: p.level } : {}), // rooftop level (omitted on the ground to save bytes)
      });
    }
    return out;
  }
