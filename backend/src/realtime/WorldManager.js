  ...
  _buildSurfaceCrowdConfig(mapData, planet) {
    ...
    for (let ty = 0; ty < tm.tiles.length; ty++) {
      ...
      for (let tx = 0; tx < row.length; tx++) {
        ...
        const pt = { x: (tx + 0.5) * ts, y: (ty + 0.5) * ts };
        if (t.type === 'plaza') plazas.push(pt);
        else if (t.type === 'street') streets.push(pt);
      }
    }
    ...
    for (const w of this.worlds.values()) {
      ...
      for (const p of w.players.values()) {
        ...
        ws.send(JSON.stringify({
          ...
          self: { x: r2(p.x), z: r2(p.z), f: r2(p.facing), l: p.roofLevel || 0, hp: p.combatant ? p.combatant.stats.health : p.maxHp, maxHp: p.maxHp, dead: p.dead ? 1 : 0 },
          players,
          enemies,
          ...(crowd ? { crowd } : {}),
          fx,
