...
describe('questService.checkPrerequisites - faction reputation', () => {
  test('passes when reputation meets the minimum', () => {
    const quest = { prerequisites: { reputation: { new_republic: 50 } } };
    expect(questService.checkPrerequisites(quest, character, [], { new_republic: 60 })).toBe(true);
  });

  test('blocks when reputation is below the minimum', () => {
    const quest = { prerequisites: { reputation: { new_republic: 50 } } };
    expect(questService.checkPrerequisites(quest, character, [], { new_republic: 40 })).toBe(false);
  });

  test('treats a missing faction as 0 reputation', () => {
    const quest = { prerequisites: { reputation: { imperial_remnant: 30 } } };
  ...
  test('requires ALL listed factions to meet their minimums', () => {
    const quest = { prerequisites: { reputation: { new_republic: 50, jedi_seekers: 20 } } };
    expect(questService.checkPrerequisites(quest, character, [], { new_republic: 60, jedi_seekers: 10 })).toBe(false);
    expect(questService.checkPrerequisites(quest, character, [], { new_republic: 60, jedi_seekers: 25 })).toBe(true);
  });

  test('still enforces level and completed-quest prerequisites alongside reputation', () => {
    const quest = { prerequisites: { level: 12, reputation: { new_republic: 10 } } };
    expect(questService.checkPrerequisites(quest, character, [], { new_republic: 99 })).toBe(false); // level 10 < 12
