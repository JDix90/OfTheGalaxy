...
const naiveGear = { weaponId: 'pulser_pistol_01', armorId: 'armor_light_01' };
const optGearByLevel = (L) => L >= 12 ? { weaponId: 'pulser_rifle_01', armorId: 'armor_heavy_01' } : L >= 6 ? { weaponId: 'pulser_rifle_01', armorId: 'armor_medium_01' } : { weaponId: 'pulser_pistol_01', armorId: 'armor_light_01' };
...
console.log('\n========== 3. MATCHED-LEVEL FIGHTS vs Ironclad (moderate) ==========');
...
for (const [buildName, statFn, gearFn] of [['naive', naiveStats, () => naiveGear], ['optimized', optStats, optGearByLevel]]) {
  for (const L of [1, 3, 5, 8, 12, 16, 20]) {
    ...
    const foe = makeEnemy('ironclad', L, 'moderate');
...
for (const id of ['wild_animal', 'ironclad', 'pirate', 'syndicate_thug', 'droid_security', 'bounty_hunter', 'pirate_captain', 'ironclad_sergeant']) {
...
console.log('\n========== 5. PLAYER L10 vs MISMATCHED ENEMY LEVELS (Ironclad) ==========');
...
for (const eL of [5, 8, 10, 13, 16, 20]) {
  const foe = makeEnemy('ironclad', eL, 'moderate');
...
for (const L of [1, 5, 10]) {
  const foe = makeEnemy('ironclad', L, 'moderate');
  console.log(`L${L} ironclad: ${foe.creditsReward} credits, ${foe.xpReward} XP per kill`);
}
const sampleItems = ['pulser_pistol_01', 'pulser_rifle_01', 'armor_light_01', 'armor_medium_01', 'armor_heavy_01'];
...
console.log('\n========== 7. PACING: fights-to-next-level (moderate ironclad) ==========');
console.log('Lvl | xpToNext | xp/kill | kills to level');
for (const L of [1, 3, 5, 8, 12, 16]) {
  const xpKill = makeEnemy('ironclad', L, 'moderate').xpReward;
