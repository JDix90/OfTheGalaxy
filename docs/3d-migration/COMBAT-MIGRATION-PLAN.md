...
| **6 — Tutorial → 3D scripted fight** | make the spaceport real-time, then tutorial drone + 3D combat-step redesign (old route kept as fallback) | 🔨 **In progress** (branch `feat/combat-migration-phase-6-tutorial`) — 6a backend ✅ (spaceport real-time, ambient toggle); 6a frontend + 6b tutorial pending. See `PHASE-6-HANDOFF.md` |
