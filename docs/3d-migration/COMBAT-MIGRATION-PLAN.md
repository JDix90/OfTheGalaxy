...
| **8 — Faction rep + polish (flagged)** | rep-on-kill (MP-aware), `defeat_boss` hook; enemy abilities **deferred**, companion actor **dropped** | ✅ **DONE (core)** (PR pending, branch `feat/combat-migration-phase-8-faction-rep`) — **8.1 Faction rep on kill:** enemy templates tagged with canonical faction ids (`iron_dominion`/`smugglers`/`vorr`/`the_tally`; `droid_security`/`wild_animal` stay `null`; the two legacy `faction:'empire'` tags — not a canonical id — normalized to `iron_dominion`). New `config/combatReputation.js` (pure: `repDeltaForKill` tier-scaled NEGATIVE deltas normal −2 / veteran −4 / elite −8 / boss −12, gated by `FACTION_REP_ON_KILL` env, default ON, read live). New `combatService.applyFactionReputationForKills(encounter)` hooked into the `endEncounter('won')` funnel: keeps only **dead, faction-tagged** enemies, accumulates per-faction, calls `factionService.applyReputationChange` (best-effort per faction — a rep failure can't roll back XP/credits/loot or wedge the encounter), folds the summary into `metadata.rewards.reputation`. Killer-only MP attribution is **free** (realtime `finalize` builds `combatants` from that player's own `engagedEnemies`). Surfaced in the victory toast (`combat.js` `t:'reward'` → `netClient` → `CombatToasts`); the backend sends the display **name** so the HUD needs no faction registry. **8.3 `defeat_boss` hook:** `checkCombatAchievements` credits `defeat_boss` when a defeated enemy `isBossEnemy` (`isBoss \|\| tier==='boss' \|\| tier==='elite'`) — reachable in normal play (elite pirate captain / bounty hunter), extensible via `spawnScriptedEnemy({ isBoss:true })`. **Deferred/dropped:** 8.2 enemy abilities/telegraphs **deferred** (large; the engine already has dodge/cooldowns — separate effort); 8.4 companion actor **dropped** (resolves O4: turn-based companion combat retired in Phase 7; no 3D parity planned). 14 DB-free + 7 DB-backed tests green; frontend build clean; adversarially reviewed. |
...
**Still open:** O5 enemy abilities/telegraphs (Phase 8.2 — DEFERRED, not a blocker; tracked as a future enhancement). _Resolved:_ O1 faction-rep deltas ✅ (8.1), O2 encounter pacing ✅ (Phase 4), O3 dungeon respawn ✅ (Phase 1), O4 companion combat ✅ DROPPED (Phase 7/8), O6 `defeat_boss` hook ✅ (8.3).
...
## Open Questions
- **O1 — Faction-rep policy.** ✅ **RESOLVED (Phase 8.1):** kills DO move rep — tier-scaled NEGATIVE deltas (normal −2 / veteran −4 / elite −8 / boss −12) against the slain enemy's faction, accumulated per faction, killer-only, gated by `FACTION_REP_ON_KILL` (default ON). No positive "rival-faction" rep gain (kept simple; revisit if factions become rivalrous). Magnitudes are deliberately small so a stray grunt is a rounding error but farming a faction has consequences.
...
- **O4 — Companion/escort combat in 3D.** ✅ **RESOLVED (Phase 8 — DROPPED):** the turn-based companion combatant (`buildNPCCompanionCombatant`/`executeCompanionTurn`) was retired with the rest of the turn-based layer in Phase 7; no 3D companion actor is planned. Escorts still exist as a difficulty/escalation signal (Phase 4), just not as combatants.
- **O5 — Enemy abilities/telegraphs / boss behavior.** **DEFERRED (Phase 8.2 not built):** RT enemies remain melee-with-dodge/cooldowns; old `executeEnemyTurn` was also always-attack so parity holds. Abilities/telegraphs are a separate effort tracked as a future enhancement, not a migration blocker.
- **O6 — `defeat_boss` achievement.** ✅ **RESOLVED (Phase 8.3):** `checkCombatAchievements` credits `defeat_boss` when a defeated enemy is a boss (`isBoss \|\| tier==='boss' \|\| tier==='elite'`) — reachable in normal play via elite foes, extensible via `spawnScriptedEnemy({ isBoss:true })`.
