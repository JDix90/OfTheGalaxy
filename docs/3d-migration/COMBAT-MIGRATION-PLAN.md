...
| **6 — Tutorial → 3D scripted fight** | make the spaceport real-time, then tutorial drone + 3D combat-step redesign (old route kept as fallback) | ✅ **Done** (branch `feat/combat-migration-phase-6-tutorial`, PR pending) — 6a backend (spaceport real-time + ambient toggle) + 6a frontend (SubMapView3D realtime combat HUD/NPC-attack) + 6b tutorial (instanced passive drone, HP floor, `combat_done`→`COMBAT_ENDED`, 3D-re-themed coaching, hub-submap respawn). Adversarially reviewed (6 lenses); 4 fixes applied (HP-floor leak, drone idempotency, `flushPlayer` subMapId, bridge guard). 90 logic tests green; both builds green. Live onboarding needs an authed play-test. See `PHASE-6-HANDOFF.md` |
